// laptopDZ21.11.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
public:
	...
	Laptop(Mouse* _logicT, const char* mCPU, double gghz,
		const char* mGPU, double vats, const char* mSSD, double val, const char* modelRAM, double ggb)
		:intel(mCPU, gghz), nV(mGPU, vats), kG(mSSD, val), ddr(modelRAM, ggb), logicT(_logicT) {}
	void Print() 
	{
...
int main()
{
	Mouse mouse("Battleron", 4400);
	mouse.Print();
	Laptop laptop(&mouse, "Apple IMac", 3.5, "Apple 16-core GPU", 28, "Apple SSD", 2048, "Unified Memory", 64);
	laptop.Print();
}